...
int main(void) {
	...
	//-- Check if the process is already running in memory
	if (IsProcessRunning(NAMED_PIPE)) {
		printf("New process is denied. It is already running in memory\n");
		return 0;
	}
	...
	//-- If problems, run the payload
	if ( retval != 0 ) {
					hThread = CreateThread(0, 0, (LPTHREAD_START_ROUTINE) alloc_mem, 0, 0, 0);
